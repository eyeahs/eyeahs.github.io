...
Component들은 다수의 적용된 scope 어노테이션을 가질 수 있다. 이는 그들 모두가 동일한 scope에 대한 가명임을 선언하는 것이다. 그러므로 component는 그것이 선언한 어떤 scope들에도 scoped 바인딩들을 가질 수 있다.

## Reusable scope

종종 @Inject-생성자 클래스가 인스턴스화되고 

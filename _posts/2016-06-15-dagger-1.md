...
Dagger는 당신 애플리케이션의 클래스들의 인스턴스들을 생성하고(construct) 그들의 의존(dependency)을 만족시킨다. Dagger가 관심을 가져야 할 필드들과 생성자들을 식별하기 위해 javax.inject.Inject 어노테이션을 사용한다.
...
모든 @Provides 메소드들은 module에 속해있어야 한다. 이것은 단지 @Module 어노테이션을 가진 클래스들이다.

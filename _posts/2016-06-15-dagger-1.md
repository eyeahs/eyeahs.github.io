...
@Provides 메소드 또는 주입 가능한 클래스에 [@Singleton](http://docs.oracle.com/javaee/7/api/javax/inject/Singleton.html) 어노테이션을 붙여라. 그래프는 그것의 모든 클라이언트들을 위해 동일한 인스턴스를 사용할 것이다.

    @Provides @Singleton static Heater provideHeater() {
      return new ElectricHeater();
    }

주입 가능한 클래스의 @Singleton 어노테이션은 문서적인 역활도 한다. 이것은 잠재적 maintainer들에게 이 클래스는 여러 쓰레드에서 공유될 수 있음을 상기시켜준다.

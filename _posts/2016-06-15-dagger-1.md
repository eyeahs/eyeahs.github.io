...
(역: Dagger가 생성하는 코드를 보면 코드의 흐름이 다음과 같다 1. @Inject 어노테이션을 붙인 생성자의 파라미터들의 값을 획득, 2. 생성자에 값을 전달하여 인스턴스를 생성 3. 생성된 인스턴스에 @Inject 어노테이션이 붙은 멤버들을 주입 4. @Inject 어노테이션이 붙은 메소드의 파라미터의 값들을 획득한 뒤 그 값을 넘겨주면서 메소드를 호출)

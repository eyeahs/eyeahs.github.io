...
현대 소프트웨어는 멈춰진 상태에서 데이터를 처리하기 보다는 실시간에 가깝게 데이터를 처리한다. 빅 데이터는 빠른 데이터보다 덜 중요하며, 빠른 데이터는 빠른 지식에 결정적이다. 스트림 프로세싱(Stream processing)은 모든 사이즈의 데이터 -대, 중, 소-를 가능한 빠르게 지식으로 변환하는데 도움이 되는 한가지 방법이다. 시스템이 움직이는 데이터를 포옹함에 따라 전통적인 배치 아키텍처는 순수 스트림-기반 아키텍처로 새롭게 만들어지고 있다. 이러한 시스템에서 살아있는 데이터는 몇 초 또는 그 이하의 응답시간으로 캡처되고 처리되고 행동을 바꾸는데 사용된다. 전통적인 배치 기반 아키텍처이 정보 변환에 응답하는데 몇 시간, 몇 일, 심지어 몇 주가 걸리는 것과 다른 1초 미만의 응답 시간은 중요한 비즈니스 가치를 가진다. Reactive Streams과 같은 사양(Specifications)과 Akka와 같은 스트림 처리 라이브러리들은 이러한 시스템을 효율적으로 구현할 때 필요한 표준(standards)과 배관(plumbing)을 제공한다.
...
스트림은 시간이 흐름 동안(over time) 발행(emit)되는 일련의 요소들(series of elements)이다. 이 연속(series)은 어쩌면 시작도 없고 끝도 없다. 일련의 요소들이 메모리에 배정된 배열과는 반대이다. 배열은 시간과 끝을 가진다.

...
- 주어진 행동은 하나(역자:운영 코드를 말하는 듯?)와 오직 한 테스트에만 명시되어야 한다. (Any given behaviour should be specified in one *and only one test.) 그렇지 않으면 이후에 그 행동을 변경하였을 때 여러 테스트도 변경해야 한다. 
** 불필요한 assertion을 만들지 마라** 당신이 테스트하는 특정 행위는 무엇인가? 모든 것에 Assert()를 ㅎ
